...
function submit_login(){
	...
	$this->form_validation->set_rules('txtPassword', 'Passwords', 'required|trim|xss_clean|callback_pword_check');
	{
		...
		$outlet = $this->get_outlet('outlet', array('id' => $data['outlet_id']))->row();
		
		$data1['outlet_id'] = $outlet->id;
		$data1['outlet_name'] = $outlet->building_name;
		$data1['longitude'] = $outlet->longitude;
		$data1['latitude'] = $outlet->latitude;

		$this->session->set_userdata('user_data', $data);
		// $this->session->set_userdata('outlet_data', $data1);
...
function submit_reset_pword(){
	...
	if ($this->form_validation->run($this) == FALSE){
		...
		$result = Modules::run("users/_update_where",$where,$data);
		if($result){
			$data['code'] = Modules::run("site_security/get_encrypt_code");
			$email['email'] = $this->session->userdata('email');
			Modules::run("users/_update_where",$email,$data);
...
function get_outlet($table,$where){
$this->load->model('mdl_login');
return $this->mdl_login->get_outlet($table,$where);
}
}