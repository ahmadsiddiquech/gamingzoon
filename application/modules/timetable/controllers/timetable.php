...
Modules::run('site_security/has_permission');
    ...
    function print_timetable(){
        ...
        $user_data = $this->session->userdata('user_data');
        $outlet_id = $user_data['outlet_id'];
        $timetable = $this->_get_timetable_print_voucher($timetable_id,$outlet_id)->result_array();
        $org = $this->_get_org_print_voucher($outlet_id)->result_array();
        $data['org'] = $org;
    ...
    function submit() {
            ...
            if ($update_id != 0) {
                ...
                $this->adding_timetable_subject($subject_name, $start_time, $end_time,$timetable_id,$user_data['outlet_id'] ,$break,$break_start_time,$break_end_time);

                $no_of_days = $this->_no_of_days($data['class_id'],$data['section_id'])->num_rows();
                if ($no_of_days == 6) {
                    $whereSection['section_id'] = $data['section_id'];
                    $parents = $this->_get_parent_id_for_notification($whereSection,$data['outlet_id'])->result_array();
                    if (isset($parents) && !empty($parents)) {
                        foreach ($parents as $key => $value) {
                            $data2['outlet_id'] = $value['parent_id'];
                            ...
                            $token = $this->_get_parent_token($value['parent_id'],$data2['outlet_id'])->result_array();
                            Modules::run('front/send_notification',$token,$nid,$data2['notif_title'],$data2['notif_description']);
                        }
                    }
                    $teachers = $this->_get_teacher_id_for_notification($whereSection,$data['outlet_id'])->result_array();
                    if (isset($teachers) && !empty($teachers)) {
                        foreach ($teachers as $key => $value) {
                            $data2['notif_for'] = 'Teacher';
                            $data2['outlet_id'] = $value['teacher_id'];
                            $data2['std_id'] = '';
                            $data2['std_name'] = '';
                            $data2['std_roll_no'] = '';
                            $data2['notif_title'] = 'Timetabe for Class '.$data['class_name'];
                            ...
                            $data2['outlet_id'] = $data['outlet_id'];
                            $nid = $this->_notif_insert_data($data2);
                            $token = $this->_get_teacher_token($value['teacher_id'],$data2['outlet_id'])->result_array();
                            Modules::run('front/send_notification',$token,$nid,$data2['notif_title'],$data2['notif_description']);
                        }
                    }
                }
    ...
    function adding_timetable_subject($subject_name ,$start_time,$end_time,$timetable_id,$outlet_id,$break,$break_start_time,$break_end_time) {
        $counter=0;
    ...
    function _update($arr_col, $outlet_id, $data) {
        $this->load->model('mdl_timetable');
        $this->mdl_timetable->_update($arr_col, $outlet_id, $data);
    }
    ...
    function _delete($arr_col, $outlet_id) {       
        $this->load->model('mdl_timetable');
        $this->mdl_timetable->_delete($arr_col, $outlet_id);
    }
    ...
    function _get_parent_id_for_notification($where,$outlet_id){
        $this->load->model('mdl_timetable');
        return $this->mdl_timetable->_get_parent_id_for_notification($where,$outlet_id);
    }

    function _get_teacher_id_for_notification($where,$outlet_id){
        $this->load->model('mdl_timetable');
        return $this->mdl_timetable->_get_teacher_id_for_notification($where,$outlet_id);
    }

    function _get_teacher_token($teacher_id,$outlet_id){
        $this->load->model('mdl_timetable');
        return $this->mdl_timetable->_get_teacher_token($teacher_id,$outlet_id);
    }

    function _get_parent_token($parent_id,$outlet_id){
        $this->load->model('mdl_timetable');
        return $this->mdl_timetable->_get_parent_token($parent_id,$outlet_id);
    }

    function _get_timetable_print_voucher($timetable_id,$outlet_id){
        $this->load->model('mdl_timetable');
        return $this->mdl_timetable->_get_timetable_print_voucher($timetable_id,$outlet_id);
    }

    function _get_org_print_voucher($outlet_id){
        $this->load->model('mdl_timetable');
        return $this->mdl_timetable->_get_org_print_voucher($outlet_id);
    }
}
?>